...
$(function () {
    ...
    $(document).on("click", "#addNewEmail", function () {
        ...
        // add also the checkbox to generate for each object here!!
        if (validateEmail(emailAddress)) {
            ...
            '<span class="m-l-20">Primary</span>'+
            '<input type="checkbox" class="m-l-10 emailCheckbox"/>' +
            ...
            //add the removal here so that on clicking the add button the object is removed as long as its validated.
            $('#emailNotSubmittedFeedback').remove();
        } else {
    ...
    //Add a new change method here to remove the primary button from other emails so only one email is always true or no email is clicked. But there cannot be two emails clicked
    $(document).on('change', '.emailCheckbox', function () {
        if ($(this).is(':checked')) {
            $('.emailCheckbox').not(this).prop('checked', false);
        }
    });
    //Here is where you create alert to add email before you submit
    $(document).on("click", "#saveContactButton", function () {
        ...
        function getEmailAddresses() {
            return $(".emailListItem").map(function () {
                return {
                    ...
                    Type: $(this).data("type"),
                    //add here the new boolean value of IsChecked so its updated in the backend via AJAX when it is called.
                    IsChecked: $(this).find("input[type='checkbox']").prop("checked")
                }
            }).get();
        }
        function emailAddedValidation() {
            let emailCheck = $('#newEmailAddress').val();
            let isValid = true;
            $('.invalidMessage').hide();
            $('.form-control').removeClass("invalidInput");
            if (emailCheck !== "") {
                $('#newEmailAddress').addClass("invalidInput");
                let $emailFeedback = $('<div>', {
                    id: 'emailNotSubmittedFeedback',
                    class: 'invalidMessage',
                    text: 'Please remember to add an email.',
                    style: 'display: block;'
                });
                $('#newEmailAddress').after($emailFeedback);
                isValid = false;
            }
        ...
        //Should go here because it is the function that is called to validate before submitting the AJAX post request probably better make a function for it.
        ...
        if (validateInputs(data) && emailAddedValidation()){
            $.ajax({
                type: "POST",
